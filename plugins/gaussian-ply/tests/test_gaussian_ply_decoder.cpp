...
void TestHighDegreeShLayout(const char* fixture, int degree)
{
    ...
    for (std::size_t c = 0; c < restCount; ++c) {
        ...
        CHECK(CloseF3(cloud.restCoefficients[c], flip * (1.0f + c),
                      flip * (101.0f + c), flip * (201.0f + c)));
    }
...
void TestReorderedProperties()
{
    ...
    std::string error;
    CHECK(decoder.Decode(Fixture("degree-1-sh.ply"), &canonical, nullptr,
                         &error));
    CHECK(decoder.Decode(Fixture("reordered-properties.ply"), &reordered,
                         nullptr, &error));
    CHECK(canonical.gaussianCount == reordered.gaussianCount);
    CHECK(canonical.shDegree == reordered.shDegree);
    CHECK(canonical.restCoefficients.size() ==
          reordered.restCoefficients.size());
    ...
    CHECK(CloseF3(reordered.positions[0], canonical.positions[0].x,
                  canonical.positions[0].y, canonical.positions[0].z));
...
void TestThreeGaussians()
{
    ...
    gs::GaussianCloudData cloud;
    std::string error;
    CHECK(decoder.Decode(Fixture("three-gaussian-binary-le.ply"), &cloud,
                         nullptr, &error));
    ...
    CHECK(CloseF3(cloud.scales[0], 1.0f, std::exp(0.5f), std::exp(1.0f)));
    CHECK(CloseF3(cloud.scales[1], std::exp(-1.0f), 1.0f, std::exp(0.25f)));
    CHECK(CloseF3(cloud.scales[2], std::exp(2.0f), std::exp(-0.5f), 1.0f));
...
void TestMetadata()
{
    ...
    CHECK(
        decoder.DecodeMetadata(Fixture("degree-3-sh.ply"), &metadata, &error));
    CHECK(metadata.gaussianCount == 1);
    CHECK(metadata.shDegree == 3);

    CHECK(decoder.DecodeMetadata(Fixture("three-gaussian-binary-le.ply"),
                                 &metadata, &error));
    ...
    error.clear();
    CHECK(!decoder.DecodeMetadata(Fixture("duplicate-sh-index.ply"), &metadata,
                                  &error));
...
void TestFailures()
{
    ...
    for (const auto& [fixture, code] : expectedCodes) {
        ...
        std::string error;
        CHECK(
            !decoder.Decode(Fixture(fixture.c_str()), &cloud, nullptr, &error));
    ...
    // Substring stability for the most user-facing messages.
    {
        gs::GaussianCloudData cloud;
        std::string error;
        CHECK(!decoder.Decode(Fixture("mesh-not-gaussian.ply"), &cloud, nullptr,
                              &error));
        CHECK(error.find("not a supported Gaussian") != std::string::npos);
        error.clear();
        CHECK(!decoder.Decode(Fixture("missing-opacity.ply"), &cloud, nullptr,
                              &error));
        ...
        CHECK(!decoder.Decode(Fixture("truncated-binary-le.ply"), &cloud,
                              nullptr, &error));
        CHECK(error.rfind("[GSPLY-E", 0) == 0);
        error.clear();
        CHECK(!decoder.Decode(Fixture("out-of-range-double.ply"), &cloud,
                              nullptr, &error));
...
void TestImportOptionParsing()
{
    ...
    CHECK(gs::ply::ParseImportOptions({{"shDegree", "2"},
                                       {"opacityThreshold", "0.25"},
                                       {"scaleMultiplier", "1.5"},
                                       {"unrelatedHostArg", "x"}},
                                      &options, &error));
    ...
    for (const auto& arguments : badValues) {
        ...
        if (gs::ply::ParseImportOptions(arguments, &options, &error) ||
            ...
            std::cerr << "expected GSPLY-E201 for '" << arguments.begin()->first
                      << "' = '" << arguments.begin()->second
                      << "', got: " << error << "\n";
            ++failures;
        }
    }
...
void TestImportOptionApplication()
{
...

    {
        ...
        CHECK(CloseF3(cloud.scales[0], 2.0f, 2.0f * std::exp(0.5f),
                      2.0f * std::exp(1.0f)));
    }
...

    {
        ...
        for (std::size_t i = 0; i < cloud.gaussianCount; ++i) {
            for (std::size_t c = 0; c < oldRest; ++c) {
                ...
                cloud.restCoefficients[i * oldRest + c] = {base, base + 0.25f,
                                                           base + 0.5f};
            }
        }
        gs::ply::GaussianPlyImportOptions options;
        ...
        for (std::size_t i = 0; i < 2; ++i) {
            for (std::size_t c = 0; c < 3; ++c) {
                const float base = 100.0f * i + c;
                CHECK(CloseF3(cloud.restCoefficients[i * 3 + c], base,
                              base + 0.25f, base + 0.5f));
            }
        }
    }
